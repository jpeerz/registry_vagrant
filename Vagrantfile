Vagrant.configure("2") do |config|
  ...
  config.vm.define "jenkins_master", autostart: false do |jenkins_master|
    ...
    # Enable the Puppet provisioner, with will look in manifests
    jenkins_master.vm.provision :puppet do |puppet|
      puppet.manifests_path = "puppet/manifests"
      puppet.manifest_file = "jenkins_master_default.pp"
      puppet.module_path = "puppet/modules"
      puppet.options = "--verbose --debug"
    end

    # Every Vagrant virtual environment requires a box to build off of.
    jenkins_master.vm.box = "ubuntu/trusty64"

    # Forward guest port to host port and name mapping
    jenkins_master.vm.network :forwarded_port, guest: 8383, host: 8383
